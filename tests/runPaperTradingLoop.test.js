...
test('loop parseArgs forwards advanced one-shot flags AND its own scheduling flags', () => {
  const a = parseArgs([
    ...
    '--enabled-providers', 'alpaca,benzinga',
    '--max-events-classified-per-cycle', '12',
    '--max-qualifying-events-attempted-per-cycle', '6',
    '--provider-cooldown-minutes', '20',
    '--provider-max-failures-before-cooldown', '3',
    '--duplicate-story-window-minutes', '15',
  ...
  assert.equal(a.maxEventsClassifiedPerCycle, 12);
  assert.equal(a.maxQualifyingEventsAttemptedPerCycle, 6);
  assert.equal(a.providerCooldownMinutes, 20);
  assert.equal(a.providerMaxFailuresBeforeCooldown, 3);
  assert.equal(a.duplicateStoryWindowMinutes, 15);
  assert.deepEqual(a.enabledProviders, ['alpaca', 'benzinga']);
